...
/** Cloudinary: smaller/faster + preload (kills 700–800KB feel) */
...
function FullSplitSection({
  ...
  /** true = allow image render when loaded; HERO uses showMeta */
  imageGate?: boolean;
  imgUrl: string;
  imgPos?: string;
  paperUrl: string;
...
export default memo(function UseCaseRetail() {
  ...
  // images (SƏNİN 4 ŞƏKİL) — keep original base URLs, transform in cldMain()
  const BG_1 =
    "https://res.cloudinary.com/dppoomunj/image/upload/v1771226758/harryjburgess-hallway-3341001_1920_agm7xv.jpg";
  const BG_2 =
    "https://res.cloudinary.com/dppoomunj/image/upload/v1771226769/maxmann-shopping-2789216_1920_tlsohv.jpg";
  const BG_3 =
    "https://res.cloudinary.com/dppoomunj/image/upload/v1771226749/as_photography-shopping-mall-1316787_1920_jtcyhx.jpg";
  const BG_4 =
    "https://res.cloudinary.com/dppoomunj/image/upload/v1771226920/1139623-escalator-897670_1920_ecobun.jpg";

  // ✅ preload: hero + others (reduces “late pop-in”)
  useEffect(() => {
    preloadImages([
      cldMain(BG_1, 1600),
      cldMain(BG_2, 1200),
  ...
  const t = useMemo(() => {
    return {
      cta1: lang === "az" ? "Mağazaya uyğunlaşdır" : "Adapt",
      cta2: lang === "az" ? "Xidmətlər" : "Services",
      s1: {
        eyebrow: "USE CASE • RETAIL",
        title: "Retail senarisi",
        lead: "Müştəri yazır → sistem niyyəti anlayır → doğru addımı verir.",
        bullets: [
          "Məhsul sorğusu: ölçü/qiymət/stok → next-step.",
          "Kampaniya/kupon → uyğun təklif + CTA.",
          "Hər addım ölçülür (conversion, cart, checkout).",
        ],
        chips: [
          { icon: ScanSearch, label: "Sorğu" },
          { icon: ClipboardList, label: "Kataloq" },
          { icon: CalendarCheck2, label: "Checkout" },
      ...
      s2: {
        ...
        title: "Sürətli seçim",
        lead: "2–3 sual → filter → uyğun məhsul → CTA.",
        bullets: ["Kateqoriya + büdcə → uyğun seçimlər.", "Ölçü/rəng/stok → konkret nəticə.", "Həssas mövzu → operator."],
        chips: [
          { icon: ScanSearch, label: "Intent" },
          { icon: ClipboardList, label: "Filter" },
      ...
      s4: {
        ...
        bullets: ["Minimal data + log/audit.", "KPI: response, conversion, handoff.", "Optim: A/B test."],
        chips: [
  ...
  return (
    <main ref={rootRef as any} className="fsRoot">
      ...
      {/* Other sections: scroll reveal əvvəlki kimi (şəkil normal görünür) */}
      <FullSplitSection
        flip
        imgUrl={BG_2}
        imgPos="center"
        ...
        chips={t.s2.chips}
        cta1Href={toContact}
        cta1Label={lang === "az" ? "Seçimi quraq" : "Set up discovery"}
        cta2Href={toServices}
        cta2Label={t.cta2}
      />
