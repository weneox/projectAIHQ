...
function normalizeList(values = []) {
  return arr(values).map((value) => lower(value)).filter(Boolean);
}
...
function FilterMenuShell({ children }) {
    ...
    <div className="p-1.5">
      <div className="team-filter-control flex h-8 min-w-0 items-center gap-2 rounded-[7px] bg-white px-2.5 text-text-muted transition-[background-color,box-shadow] duration-150 ease-premium focus-within:text-text">
        ...
        <input
          ...
          autoCapitalize="none"
          spellCheck={false}
          className="h-full min-w-0 flex-1 appearance-none border-0 bg-transparent p-0 text-[12.5px] font-semibold text-text outline-none ring-0 placeholder:text-text-subtle focus:border-0 focus:outline-none focus:ring-0 focus-visible:outline-none"
        />
      </div>
    </div>
  );
...
function FilterAction({ children, onClick, disabled = false }) {
  return (
    <button
      type="button"
      onClick={onClick}
...
function MultiSelectMenu({ options, selectedValues, onToggle, onClear, allLabel }) {
  const selected = normalizeList(selectedValues);

  return (
    <FilterMenuShell>
      <FilterOption selected={!selected.length} onClick={onClear}>
        {allLabel}
      </FilterOption>
  ...
  return (
    <div className="team-soft-control flex h-11 items-center rounded-[12px] bg-white px-3.5 transition-[background-color,box-shadow] duration-150 ease-premium">
      <input
        ...
        spellCheck={false}
        name={name || undefined}
        className="h-full min-w-0 flex-1 appearance-none border-0 bg-transparent p-0 text-[13.5px] font-semibold text-text outline-none ring-0 placeholder:text-text-subtle disabled:cursor-not-allowed focus:border-0 focus:outline-none focus:ring-0 focus-visible:outline-none"
      />
    </div>
  );
}
...
function ChoiceGroup({ label, value, options, disabled = false, onChange }) {
  return (
              ...
              {arr(state.users).length ? (
                <div className="relative overflow-visible">
                  <div className={TABLE_MIN_WIDTH}>
                    <div className="relative z-20 border-b border-line-soft px-0 py-2">
                      <div
                        className={cx(
                          "grid",
                          TABLE_MIN_WIDTH,
