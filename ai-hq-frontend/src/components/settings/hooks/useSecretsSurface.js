...
export function useSecretsSurface({ canManage = false, initialProvider = "meta" } = {}) {
  ...
  const saveSecret = useCallback(async () => {
    ...
    if (!p) {
      failSave("Provider is required.");
      return null;
    }

    if (!k) {
      failSave("Secret key is required.");
      return null;
    }

    if (!v) {
      failSave("Secret value is required.");
      return null;
    }
