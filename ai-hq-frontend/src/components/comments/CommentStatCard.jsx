export default function CommentStatCard({ label, value, icon: Icon, tone = "neutral" }) {
  const toneMap = {
    ...
    amber: "premium-stat border-amber-200/90 bg-[linear-gradient(180deg,rgba(255,250,241,0.94),rgba(253,245,227,0.84))]",
  };

  return (
    <div className={`p-5 ${toneMap[tone] || toneMap.neutral}`}>
      <div className="flex items-start justify-between gap-4">
        <div>
          <div className="text-[11px] uppercase tracking-[0.2em] text-slate-400">{label}</div>
          <div className="mt-3 text-[30px] font-semibold tracking-[-0.045em] text-slate-950">{value}</div>
        </div>
