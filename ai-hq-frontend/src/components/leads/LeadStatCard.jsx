...
export default function LeadStatCard({ label, value, icon: Icon, tone = "neutral" }) {
  const toneMap = {
    ...
    violet: "border-line bg-surface-muted",
  };

  return (
    <div
      className={`rounded-lg border p-5 ${toneMap[tone] || toneMap.neutral}`}
    >
      <div className="flex items-start justify-between gap-4">
        <div>
          ...
          <div className="mt-3 text-[30px] font-semibold tracking-[-0.03em] text-text">
            {value}
          </div>
        </div>
