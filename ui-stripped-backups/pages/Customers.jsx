...
function CustomerAvatar({ name, size = "md" }) {
  ...
  return (
    <div
      className={cx(
        "flex shrink-0 items-center justify-center rounded-md font-semibold tracking-[-0.03em] ring-1 ring-white shadow-[0_1px_2px_rgba(15,23,42,0.06),0_14px_26px_-22px_rgba(15,23,42,0.28)]",
        sizeClass,
        avatarTone(name)
      )}
    >
...
function StageLabel({ stage, pill = false }) {
  ...
  return (
    <span className="inline-flex items-center gap-2">
      <span className={cx("h-1.5 w-1.5 rounded-md", stageDotClass(tone))} />
  ...
  return (
    <div className="flex min-h-[360px] items-center justify-center px-6 py-12 text-center">
      <div className="max-w-[520px]">
        <div className="mx-auto h-1.5 w-14 rounded-md bg-[#C9D5E4]" />
        ...
        <div className="absolute left-3 top-[calc(100%+8px)] z-[220] w-[320px] overflow-hidden rounded-[18px] border border-[#D7E1EC] bg-white shadow-[0_26px_70px_-38px_rgba(15,23,42,0.38),0_12px_28px_-22px_rgba(15,23,42,0.18)]">
          <div className="border-b border-[#E5EBF3] bg-white p-2">
            <Input
              ...
              appearance="quiet"
              leftIcon={<Search className="h-[14px] w-[14px]" strokeWidth={2.15} />}
              right={
                term ? (
                  <button
                    type="button"
          ...
          <div className="max-h-[320px] overflow-y-auto py-1">
            ...
            >
              <div className="flex h-9 w-9 shrink-0 items-center justify-center rounded-md bg-[#EEF4FF] text-[12px] font-bold text-[#315CFF] ring-1 ring-white">
                All
              </div>
...
function CustomerTable({
  ...
  const stageDisplay =
    STAGES.find((item) => item.key === stageFilter)?.label || "All";
  ...
  const dateDisplay = dateFilter
    ? formatLongDate(dateFilter + "T00:00:00") || dateFilter
    : "Any date";
  ...
  function sourceText(lead = {}) {
    return titleize(s(lead.source || lead.channel || lead.channel_type || "Direct"));
  }

  function statusPillClass(status = "") {
    const safe = lower(status);

    if (["won", "customer", "converted"].includes(safe)) {
      return "bg-[#EAF8EF] text-[#147A3E] ring-[#BFE8CD]";
    }

    if (["closed", "done", "resolved"].includes(safe)) {
      return "bg-[#F8FAFC] text-[#475569] ring-[#DCE6F1]";
    }

    if (["pending", "waiting"].includes(safe)) {
      return "bg-[#FFF8E8] text-[#A16207] ring-[#F6D78B]";
    }

    return "bg-[#EEF4FF] text-[#315CFF] ring-[#DDE8FF]";
  }
    ...
    <table className="customers-filter-table w-full table-fixed border-collapse">
      ...
      <thead className="bg-white">
        <tr className="h-14 bg-white text-[10.5px] font-semibold uppercase tracking-[0.17em] text-[#66768A]">
          ...
          <th className="h-14 border-b border-l border-[#E3EAF2] !bg-white px-4 text-left font-semibold shadow-[inset_0_-1px_0_rgba(216,226,238,0.95)]">
            <div className="text-[10px] font-semibold uppercase leading-none tracking-[0.18em] text-[#8A96A8]">
              Source
            </div>
            <div className="mt-2 text-[13px] font-semibold normal-case tracking-[-0.02em] text-[#0F172A]">
              Channel
            </div>
          </th>
          ...
          <th className="relative z-[85] h-14 border-b border-l border-[#E3EAF2] !bg-white p-0 text-left font-semibold shadow-[inset_0_-1px_0_rgba(216,226,238,0.95)]">
            <button
              ...
              className="group flex h-full w-full cursor-pointer items-center gap-2.5 bg-white px-4 text-left outline-none transition-colors duration-base ease-premium hover:bg-white focus-visible:bg-white"
            >
              <Search
                className="mt-[19px] h-[14px] w-[14px] shrink-0 text-[#7C8A9F] transition-colors duration-base ease-premium group-hover:text-[#315CFF]"
                strokeWidth={2.05}
                    ...
                    >
                      <span
                        className={cx(
                          ...
                          item.key === "all"
                            ? "bg-[#94A3B8]"
                            : stageDotClass(toneForStage(item.key))
                        )}
                      />
                ...
                <div className="p-3">
                  ...
                  >
                    ...
                    <span
                      className={cx(
                        "text-[14px] font-bold",
                        !dateFilter ? "text-[#315CFF]" : "text-transparent"
                      )}
                    >
                      ?
                    </span>
          ...
          <th className="h-14 border-b border-l border-[#E3EAF2] !bg-white px-3 text-center font-semibold shadow-[inset_0_-1px_0_rgba(216,226,238,0.95)]">
            <div className="text-[10px] font-semibold uppercase leading-none tracking-[0.18em] text-[#8A96A8]">
              Open
            </div>
            <div className="mt-2 text-[13px] font-semibold normal-case tracking-[-0.02em] text-[#0F172A]">
            ...
            >
              <td className="relative px-6 py-3.5">
                <span
                  aria-hidden="true"
                  className={cx(
                    "pointer-events-none absolute left-0 top-1/2 h-12 w-[4px] -translate-y-1/2 rounded-md !bg-[#315CFF] shadow-[0_0_0_1px_rgba(49,92,255,0.08),0_0_18px_rgba(49,92,255,0.35)] transition-opacity duration-base ease-premium",
              ...
              <td className="px-4 py-3.5 align-middle">
                <div className="inline-flex max-w-full rounded-md border border-[#E3EAF2] bg-[#F8FAFC] px-2.5 py-1 text-[12px] font-semibold text-[#475569]">
              ...
              <td className="px-4 py-3.5 align-middle text-[13px] font-semibold text-[#475569]">
              ...
              <td className="px-4 py-3.5 align-middle">
                <span
                  className={cx(
                    "inline-flex rounded-md px-2.5 py-1 text-[12px] font-semibold ring-1 shadow-[inset_0_1px_0_rgba(255,255,255,0.78)]",
              ...
              <td className="px-3 py-3.5 align-middle text-center">
                {threadId ? (
                  <button
                    ...
                    className="inline-flex h-9 w-9 items-center justify-center rounded-md border border-[#D7E1EC] bg-white text-[#315CFF] shadow-[0_8px_18px_-16px_rgba(15,23,42,0.24)] transition-[border-color,transform,box-shadow,background-color] duration-base ease-premium hover:-translate-y-px hover:border-[#BDD0FF] hover:bg-[#F8FBFF] hover:shadow-[0_14px_28px_-18px_rgba(49,92,255,0.28)]"
...
function CustomerDetailPanel({ lead, onOpen, onClose, closed = false, closing = false, opening = false }) {
  ...
  if (!lead) {
    return (
      <aside className={cx("customers-detail-panel app-detail-panel hidden h-full min-h-0 overflow-y-auto bg-white px-6 pb-6 xl:block", closing && "customers-detail-panel-closing", opening && "customers-detail-panel-opening")}>
...
const selectedLead = useMemo(() => {
  ...
  return (
    <PageCanvas data-aihq-customers-page="true" className="!mx-0 !h-full !min-h-0 !w-full !max-w-none !space-y-0 overflow-hidden bg-white customers-page-white customers-page-white">
      ...
      <section className="app-data-page h-full min-h-0 overflow-hidden bg-white">
