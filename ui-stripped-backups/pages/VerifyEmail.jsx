...
export default function VerifyEmail() {
  ...
  useEffect(() => {
    ...
    async function run() {
      if (!token) {
        setStatus("code");
        setMessage({
          title: sent ? "Code sent" : "Verify email",
          description: sent
            ? "Enter the 6-digit code from your latest email."
            : "Use the 6-digit verification code from your latest email.",
        });
        return;
      }
  ...
  return (
    <div className="min-h-screen bg-white text-text">
      <main className="mx-auto flex min-h-screen w-full max-w-[520px] flex-col justify-center px-6 py-10">
        <Card padded={false} clip className="shadow-[0_28px_80px_-64px_rgba(15,23,42,0.55)]">
          <div className="px-6 py-6">
            <div className="flex items-center justify-between gap-4">
              <div className="min-w-0">
                ...
                <h1 className="mt-1 text-[21px] font-semibold tracking-[var(--tracking-tight-lg)] text-text">
                  {verified
                    ? "Email verified"
                    : verifying
                      ? "Verifying email"
              ...
              {verified ? (
                <CheckCircle2 className="h-5 w-5 shrink-0 text-success" strokeWidth={2.1} />
              ) : verifying ? (
                <RefreshCw className="h-5 w-5 shrink-0 animate-spin text-text-muted" strokeWidth={2.1} />
              ) : (
                <Mail className="h-5 w-5 shrink-0 text-text-muted" strokeWidth={2.1} />
              )}
            </div>
          ...
          {!verified && !token ? (
            <form className="border-t border-line-soft px-6 py-5" onSubmit={handleVerifyCode}>
              <Input
                ...
                appearance="large"
                inputClassName="text-center text-[22px] tracking-[0.28em] placeholder:text-[14px] placeholder:tracking-normal"
              />
          ...
          <div className="grid gap-3 border-t border-line-soft px-6 py-5">
            {verified ? (
              <Button
                type="button"
                size="md"
            ...
            <Link
              to="/login"
              className="text-center text-[13px] font-semibold text-text-muted underline underline-offset-[3px] transition-colors hover:text-text"
            >
              Back to sign in
            </Link>
          </div>
