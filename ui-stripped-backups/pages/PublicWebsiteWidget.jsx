...
export default function PublicWebsiteWidget() {
  ...
  return (
    <div className="min-h-screen bg-[radial-gradient(circle_at_top,_rgba(148,163,184,0.25),_transparent_40%),linear-gradient(180deg,_#f8fafc_0%,_#e2e8f0_100%)] p-3 text-slate-900">
            ...
            <>
              ...
              {messages.length ? (
                ...
                <div className="flex h-full flex-col items-center justify-center px-5 text-center">
                  <div className="flex h-14 w-14 items-center justify-center rounded-md bg-slate-100 text-slate-600">
                    ...
                    <div className="mt-5 flex flex-wrap justify-center gap-2">
                      {widget.initialPrompts.map((prompt) => (
                        <button
                          ...
                          className="rounded-md border border-slate-200 bg-white px-3 py-2 text-[12px] font-medium text-slate-600 transition hover:border-slate-300 hover:text-slate-900"
                        >
                          {prompt}
                        </button>
                      ))}
      ...
      textClassName="max-h-32 min-h-[24px] flex-1 resize-none bg-transparent text-[14px] leading-6 text-slate-900 outline-none placeholder:text-slate-400"
             />
            <button
              type="submit"
              disabled={!s(draft) || sending || !canSend}
              className="inline-flex h-10 w-10 items-center justify-center rounded-md bg-slate-950 text-white transition disabled:cursor-not-allowed disabled:opacity-50"
            >
              {sending ? (
