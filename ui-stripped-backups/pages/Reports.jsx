...
function RangeSwitch({ range, onChange }) {
  return (
    <div className="flex rounded-md border border-[#DDE8F3] bg-white p-1 shadow-[0_10px_26px_-24px_rgba(15,23,42,0.42)]">
      {RANGES.map((item) => (
        <button
          key={item.key}
          type="button"
          ...
          className={cx(
            "h-8 rounded-md px-3.5 text-[12px] font-semibold transition-all duration-base ease-premium",
            range === item.key
  ...
  return (
    <div className="grid gap-2">
      ...
      <div className="h-2 overflow-hidden rounded-md bg-[#EEF2F7]">
        <div
          className="h-full rounded-md bg-[#315CFF] transition-all duration-base ease-premium"
          style={{ width: width + "%" }}
        />
      </div>
    </div>
        ...
        {rows.map((row) => (
          <div key={row.label} className="px-5 py-4">
            <div className="flex items-start justify-between gap-4">
              ...
              <div className="max-w-[132px] truncate rounded-md border border-[#CDEFE9] bg-[#F0FDFA] px-2.5 py-1 text-[12px] font-semibold text-[#0F766E]">
                {row.value}
              </div>
            </div>
          </div>
...
export default function Reports() {
  ...
  return (
    <PageCanvas className="!mx-0 !h-full !min-h-0 !w-full !max-w-none !space-y-0 overflow-hidden bg-white">
      <section className="app-data-page h-full min-h-0 overflow-y-auto bg-white">
        {state.error ? (
        ...
        <div className="flex items-center justify-between gap-4 border-b border-[#E3EAF2] bg-[linear-gradient(180deg,#FFFFFF_0%,#FBFDFF_100%)] px-6 py-3">
          <div className="flex flex-wrap items-center gap-3 text-[12px] font-semibold text-[#66768A]">
            <span className="inline-flex items-center gap-1.5">
              <span className="h-2 w-2 rounded-md bg-[#315CFF]" />
              Messages
            </span>
            <span className="inline-flex items-center gap-1.5">
              <span className="h-2 w-2 rounded-md bg-[#14B8A6]" />
              AI replies
            </span>
            <span className="inline-flex items-center gap-1.5">
              <span className="h-2 w-2 rounded-md bg-[#38A3FF]" />
              Leads
            </span>
            <span className="rounded-md border border-[#DDE8F3] bg-white px-2.5 py-1 text-[#0F172A]">
              {rangeTitle(range)}
            </span>
          </div>
