...
import { useProviderSecretsSurface } from "./hooks/useProviderSecretsSurface.js";

export default function ProviderSecretsPanel({ canManage = false, permissionMessage = "" }) {
  const {
    provider,
    setProvider,
    presetKeys,
    ...
    saveSecret,
    removeSecret,
  } = useProviderSecretsSurface({ canManage });

  return (
    <Card className="p-6">
      <div className="space-y-6">
        <div>
          <h3 className="text-base font-semibold text-slate-900 dark:text-white">Provider Secrets</h3>
        ...
        <div className="grid gap-4 md:grid-cols-3">
          <div className="space-y-2">
            ...
            <select
              ...
              disabled={!canManage}
              className="h-11 w-full rounded-md border border-slate-200 bg-white px-3 text-sm text-slate-900 outline-none ring-0 transition focus:border-slate-400 dark:border-white/10 dark:bg-white/5 dark:text-white"
            >
              ...
              <div className="flex flex-wrap gap-2 pt-1">
                {presetKeys.map((keyName) => (
                  <button
                    ...
                    disabled={!canManage}
                    className="rounded-md border border-slate-200 px-2.5 py-1 text-xs text-slate-600 transition hover:border-slate-300 hover:text-slate-900 dark:border-white/10 dark:text-slate-300 dark:hover:border-white/20 dark:hover:text-white"
                  >
                    {keyName}
                  </button>
                ))}
        ...
        <div className="rounded-md border border-slate-200 dark:border-white/10">
          ...
          {surface.loading ? (
            <div className="px-4 py-6 text-sm text-slate-500 dark:text-slate-400">Loading secrets...</div>
          ) : secrets.length === 0 ? (
              ...
              {secrets.map((row) => (
                ...
                >
                  <div className="min-w-0">
                    <div className="text-sm font-medium text-slate-900 dark:text-white">{row.secret_key}</div>
                  ...
                  <div className="flex items-center gap-2">
                    <Button variant="outline" size="sm" onClick={() => setSecretKey(row.secret_key)} disabled={!canManage}>
                      Use Key
                    </Button>
                    <Button
                      variant="destructive"
