            ...
            {prompts.length ? (
              <div className="mt-4 flex flex-wrap gap-2">
                {prompts.map((prompt) => (
                  <span
                    key={prompt}
                    className="rounded-md border border-slate-200 bg-white px-3 py-2 text-[11.5px] font-semibold text-slate-600"
                  >
                    {prompt}
                  </span>
                ))}
...
function StatusBadge({ tone = "neutral", children }) {
  ...
  return (
    <Badge tone={safeTone} size="sm">
      <span className={cx("h-1.5 w-1.5 rounded-md", dotClass(safeTone))} />
      {children}
    </Badge>
  );
}
  ...
  return (
    ...
    >
      <span className="flex items-center gap-2">
        <span
          className="h-3.5 w-3.5 rounded-md border border-white shadow-[0_0_0_1px_rgba(15,23,42,0.08)]"
...
export default function WebsiteWidgetDetailDrawer({
  ...
  <div className="flex items-center justify-between gap-4">
      ...
      <div className="min-w-0">
        ...
        <div className="mt-1 flex min-w-0 items-center">
          ...
          >
            <span
              className={cx(
                "h-1.5 w-1.5 shrink-0 rounded-md",
    ...
    <button
      ...
      className="inline-flex h-10 w-10 shrink-0 items-center justify-center rounded-md border border-line-soft bg-surface text-text-muted shadow-[var(--shadow-inset-top)] transition-[background-color,border-color,color,box-shadow] duration-base ease-premium hover:border-line hover:bg-surface-subtle hover:text-text"
    >
