      ...
      {Array.isArray(history) && history.length ? (
        <div className="mt-4 space-y-4">
          {history.map((item, index) => (
            ...
            >
              <div className="absolute left-0 top-6 h-2.5 w-2.5 rounded-md bg-slate-300 first:top-2.5" />
              ...
              <div className="flex flex-wrap items-center gap-x-3 gap-y-1">
                ...
                {s(item.version) ? (
                  <div className="rounded-md border border-slate-200/80 bg-slate-50/80 px-2.5 py-1 text-[11px] font-medium uppercase tracking-[0.12em] text-slate-500">
                    {item.version}
                  </div>
                ) : null}
                {s(item.profileStatus) ? (
                  <div className="rounded-md border border-slate-200/80 bg-white/80 px-2.5 py-1 text-[11px] font-medium uppercase tracking-[0.12em] text-slate-500">
                    {item.profileStatus}
                  </div>
                ) : null}
              ...
              {typeof onOpenVersion === "function" && s(item.id) ? (
                <div className="mt-3">
                  <button
                    ...
                    className="inline-flex h-9 items-center justify-center rounded-md border border-slate-200/80 bg-white/84 px-3.5 text-xs font-medium text-slate-700 transition hover:bg-white"
                  >
                    View compare
                  </button>
                </div>
