\i ./00_extensions.sql
\i ./01_shared_functions.sql

\i ./10_tenants.sql
\i ./11_tenant_config.sql
\i ./12_tenant_access.sql
\i ./13_tenant_channels_integrations.sql
\i ./14_tenant_business_brain.sql
\i ./15_tenant_sources_and_knowledge.sql
\i ./16_tenant_services_setup.sql
\i ./17_tenant_services_patch.sql
\i ./19_auth_session_hardening.sql

\i ./20_core_threads.sql
\i ./21_core_workflow.sql
\i ./22_system_ops.sql

\i ./30_content.sql
\i ./31_media.sql

\i ./40_inbox.sql
\i ./18_fix_inbox_messages_message_type_check.sql
\i ./41_leads.sql
\i ./42_comments.sql
\i ./43_source_fusion_core.sql
\i ./44_google_maps_source_type_patch.sql
\i ./45_source_raw_artifacts.sql
\i ./46_canonical_business_graph.sql
\i ./47_runtime_projection.sql
\i ./48_schema_cleanup_deprecations.sql
\i ./49_setup_review_sessions.sql
\i ./50_voice.sql
\i ./51_canonical_truth_versions.sql
\i ./52_operational_data_backfill.sql
\i ./60_inbox_brain.sql
\i ./61_execution_policy_controls.sql
\i ./62_auth_identities.sql

\i ./90_backfill_tenant_ids.sql
\i ./91_data_repairs.sql
\i ./92_durable_execution_control_plane.sql
\i ./93_runtime_incident_trail.sql
\i ./94_operational_data_backfill_correction.sql
\i ./97_inbox_thread_latest_message_index.sql
\i ./98_website_domain_verifications.sql
\i ./99_inbox_threads_telegram_channel_check_patch.sql
\i ./100_inbox_threads_allow_telegram_channel.sql
\i ./101_tenant_decision_events.sql